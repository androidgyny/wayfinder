...
(()=>{
 ...
 function pending(value){if(!state)return;state.busy=value;$('#artwork-use').disabled=value||!state.loaded;$('#artwork-download').disabled=value;$('#artwork-play').disabled=value||!!state.searching;$('#artwork-file').disabled=value}
 function open({title,pkg,image,square,apply}){
  ...
  $('#artwork-gallery').replaceChildren();message('Start with the current image, or choose a source.');pending(false);mode('fit');showDialog('#artwork-dialog');candidate(image,'Current image');candidate('art-icon/'+pkg,'Installed icon');select(image);$('#artwork-installed').focus();searchPlay();
 }
 ...
 function select(src){if(!state)return;const current=state,token=++state.load;state.loaded=false;$('#artwork-use').disabled=true;ctx.clearRect(0,0,canvas.width,canvas.height);preview.removeAttribute('src');$('#artwork-dimensions').textContent='Loading preview…';const im=new Image();im.onload=()=>{if(state!==current||token!==state.load)return;state.image=im;state.loaded=true;state.x=state.y=.5;$('#artwork-x').value=$('#artwork-y').value='50';$('#artwork-dimensions').textContent=im.naturalWidth+' × '+im.naturalHeight+' → '+canvas.width+' × '+canvas.height;$('#artwork-use').disabled=state.busy;draw()};im.onerror=()=>{if(state!==current||token!==state.load)return;$('#artwork-dimensions').textContent='Preview unavailable';message('Could not load this image. Try another result or choose a downloaded file.')};im.src=src}
 ...
 $('#artwork-play').onclick=searchPlay;
 ...
 window.artworkController=action=>{if(!state)return false;const active=document.activeElement;let target=null;if(active===$('#artwork-url')&&action==='down')target=$('#artwork-download');else if(active===$('#artwork-download')&&action==='up')target=$('#artwork-url');else if(active?.closest('.artwork-modes')&&action==='down')target=$(state.mode==='fill'?'#artwork-x':state.mode==='compose'?'#artwork-title-toggle':'#artwork-use');if(target&&!target.disabled){controllerFocus(target);return true;}if(active!==preview)return false;if(!['left','right'].includes(action))return false;if(state.mode!=='fill')return false;const axis=action==='left'||action==='right'?'x':'y';state[axis]=Math.max(0,Math.min(1,state[axis]+(['right','down'].includes(action)?.05:-.05)));$('#artwork-'+axis).value=state[axis]*100;draw();return true};
 ...
 const previous=window.nativeEvent;window.nativeEvent=(event,data)=>{
  if(event.startsWith('artwork')){if(!dialog.open||!state||data.session!==state.session)return;
   if(event==='artworkResults'||event==='artworkSearchError'){state.searching=false;$('#artwork-play').disabled=state.busy;if(event==='artworkResults'){for(const old of document.querySelectorAll('[data-play-result]'))old.remove();for(const item of data.items){candidate(item.image,item.label,item.thumbnail);$('#artwork-gallery').lastElementChild.dataset.playResult='true'}if(!state.busy)message('Google Play images are available below.')}else if(!state.busy)message('No Google Play images found. Try SteamGridDB or Google Images.');return;}
   pending(false);
