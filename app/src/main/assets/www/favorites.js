...
function setFavoriteButton(button,g){button.disabled=!g;const on=g?.favorite===true;button.textContent=on?'★':'☆';button.setAttribute('aria-pressed',String(on));button.setAttribute('aria-label',on?'Remove from favorites':'Add to favorites');button.title=on?'Remove from favorites · R3':'Add to favorites · R3'}
...
function syncFavoritesUI(){const b=$('#favorites-section');b.classList.toggle('active',favoritesOnly);b.setAttribute('aria-pressed',String(favoritesOnly));$('#favorites-count').textContent=games.filter(g=>g.favorite===true).length;$('#presentation-sort').value=presentation==='oxford'?'az':sort;$('#empty h2').textContent=favoritesOnly&&!query?'No favorites yet':'No games found';$('#empty p').textContent=favoritesOnly&&!query?'Open a game’s details and press the star, or select a game and click the right stick.':'Try another title or browse all genres.'}
