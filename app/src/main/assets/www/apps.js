...
function drawerTile(a){const item=el('div','drawer-item'),b=el('button','drawer-app');b.dataset.package=a.package;b.setAttribute('aria-label','Open '+drawerName(a));const im=el('img');im.src=drawerImage(a);im.alt='';im.loading='lazy';b.append(im,el('span','',drawerName(a)));b.onclick=e=>{if(e.detail===0||performance.now()>pinIgnoreClickUntil)openDrawerApp(a)};b.onpointerdown=e=>startPinDrag(e,a.package);b.oncontextmenu=e=>{e.preventDefault();if(!pinDrag?.active&&performance.now()>pinIgnoreClickUntil)editDrawerApp(a.package)};const edit=el('button','drawer-edit','•••');edit.setAttribute('aria-label','Edit '+drawerName(a));edit.onclick=()=>editDrawerApp(a.package);item.append(b,edit);return item}
function renderDrawer(){
 const focusedItem=document.activeElement?.closest('.drawer-item'),focusedPackage=focusedItem?.querySelector('.drawer-app')?.dataset.package,focusedEdit=document.activeElement?.matches('.drawer-edit'),focusedIndex=focusedItem?[...$('#drawer-grid').children].indexOf(focusedItem):0,focusedTab=document.activeElement?.closest('#drawer-tabs button')?.dataset.drawerView;
 ...
 $('#drawer-tabs').replaceChildren(...[['pinned','Pinned'],['other','Other apps'],['recent','Recent'],['all','All apps']].map(([key,label])=>{const b=el('button','secondary',label);b.dataset.drawerView=key;b.setAttribute('aria-pressed',String(key===drawerView));b.onclick=()=>{drawerView=key;drawerPage=0;$('#drawer-search').value='';$('#drawer-letter').value='';renderDrawer()};return b}));
