...
let saved={};try{saved=native?JSON.parse(native.view()):{}}catch{}restoring=true;pragueKey=typeof saved.pragueKey==='string'?saved.pragueKey:'favorites';praguePositions=saved.praguePositions&&typeof saved.praguePositions==='object'?saved.praguePositions:{};ulmLevel=saved.ulmLevel==='games'?'games':'categories';ulmSection=typeof saved.ulmSection==='string'?saved.ulmSection:'all';ulmCategoryKey=typeof saved.ulmCategoryKey==='string'?saved.ulmCategoryKey:'all';ulmPositions=saved.ulmPositions&&typeof saved.ulmPositions==='object'?saved.ulmPositions:{};viennaKey=typeof saved.viennaKey==='string'?saved.viennaKey:'favorites';viennaPositions=saved.viennaPositions&&typeof saved.viennaPositions==='object'?saved.viennaPositions:{};initApps();initFavorites();initPresentation();presentation=['kyoto','cupertino','tokyo','seattle','vienna','prague','oxford','berlin','ulm','venice'].includes(saved.presentation)?saved.presentation:'library';presentationId=saved.presentationId||saved.focus||null;seattleBrowse=saved.seattleBrowse===true;document.body.dataset.presentation=presentation;$('#presentation').value=presentation;setPalette(saved.palette);soundEnabled=saved.soundEnabled!==false;$('#sound-enabled').checked=soundEnabled;setArtFit(saved.artFit);initAppearance(saved);refreshOxfordControls();rebuildGenres();favoritesOnly=saved.favoritesOnly===true;genre=favoritesOnly?'':genres.includes(saved.genre)?saved.genre:'';if(presentation==='vienna'||presentation==='prague'){genre='';favoritesOnly=false;}query=saved.query||'';sort=['az','za','recent'].includes(saved.sort)?saved.sort:'az';size=['compact','comfortable','large'].includes(saved.size)?saved.size:'comfortable';visible=Math.max(72,Math.min(saved.visible||72,games.length));$('#search').value=query;$('#sort').value=sort;setSize(size);update();setTimeout(()=>{window.scrollTo(0,saved.scroll||0);if(saved.focus&&!editingField()){const selectedCard=document.querySelector('.game[data-id="'+saved.focus+'"]');if(selectedCard){controllerGameId=saved.focus;document.body.dataset.input='controller';selectedCard.classList.add('controller-selected');selectedCard.focus({preventScroll:true})}}restoring=false;focusStartupGame()},200);
...
function focusStartupGame(){
 ...
 if(presentation==='ulm'){ulmFocus();return;}
