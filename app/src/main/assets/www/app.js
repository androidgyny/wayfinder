...
window.controller=action=>{document.querySelector('.skip')?.classList.remove('keyboard-access');if(pinDrag)finishPinDrag(true);if(copenhagenDrag?.active&&action==='back'){finishCopenhagenDrag(true);return;}finishCopenhagenDrag(true);stopCarouselMotion();
 ...
 if(!modal&&$('#grid .show-all-matches')&&!['cambridge','ulm'].includes(presentation)&&((action==='down'&&(document.activeElement===$('#search')||document.activeElement?.closest('#category-strip,#seattle-categories')))||(action==='right'&&document.activeElement?.matches('aside .genre')))){controllerFocus($('#grid .show-all-matches'),true);return;}
 if(!modal&&$('#grid .show-all-matches')&&((presentation==='vienna'&&action==='right'&&document.activeElement?.matches('.vienna-heading'))||(presentation==='prague'&&action==='down'&&document.activeElement?.matches('.prague-heading')))){controllerFocus($('#grid .show-all-matches'),true);return;}
 if(!modal&&document.activeElement?.matches('.show-all-matches')){
  if(action==='activate'){document.activeElement.click();return;}
  if((presentation==='vienna'&&action==='left')||(presentation==='prague'&&action==='up')){controllerFocus($('#grid .expanded .vienna-heading')||$('#grid .expanded .prague-heading'),true);return;}
  if(action==='up'&&!['cambridge','ulm','vienna','prague'].includes(presentation)){controllerFocus(presentation==='seattle'?$('#seattle-categories .active'):presentation==='library'?$('#search'):$('#category-current'),true);return;}
 }
...
// Search counts describe the entire library, independently of the selected section.
function refreshSearchSummary(){
 const terms=normalize(query).trim().split(/\s+/).filter(Boolean);searchSummary=null;if(!terms.length)return;
 const summary={all:0,favorites:0,recent:0,genres:new Map()};
 for(const g of games){if(!terms.every(t=>normalize(g.title).includes(t)))continue;summary.all++;if(g.favorite===true)summary.favorites++;if(g.lastPlayed>0)summary.recent++;summary.genres.set(g.genre,(summary.genres.get(g.genre)||0)+1)}searchSummary=summary;
}
function searchSectionCount(key,total){if(!searchSummary)return total;return key.startsWith('genre:')?searchSummary.genres.get(key.slice(6))||0:searchSummary[key]||0;}
function showAllSearchMatches(){
 if(presentation==='vienna'||presentation==='prague'){const list=presentation==='vienna'?viennaShelves:pragueColumns,target=list.find(s=>s.items.length);if(target){if(presentation==='vienna')viennaOpen(target.key,true);else pragueOpen(target.key,true);}return;}
 genre='';favoritesOnly=false;visible=72;
 if(presentation==='cambridge'){cambridgeOpen('all','games');return;}
 if(presentation==='ulm'){ulmSection='all';ulmCategoryKey='all';ulmLevel='games';presentationId=ulmPositions.all||null;update();ulmFocus();persist();return;}
 if(presentation==='seattle')seattleBrowse=true;
...
function syncSearchFeedback(){
 ...
 for(const b of document.querySelectorAll('.cambridge-category,.ulm-category')){const count=Number(b.lastElementChild?.textContent.replaceAll(',',''));b.classList.toggle('search-no-match',!!searchSummary&&!count);}
 const options=sections(),index=sectionIndex();
 function sectionSearchLabel(b,section){if(!b||!section||section.home)return;const count=searchSectionCount(section.favorite?'favorites':section.genre?'genre:'+section.genre:'all',0);b.textContent=section.label+(searchSummary?' · '+count:'');b.classList.toggle('search-no-match',!!searchSummary&&!count);}
 for(const [id,offset] of [['category-current',0],['category-before',-1],['category-after',1]])sectionSearchLabel(document.getElementById(id),options[(index+offset+options.length)%options.length]);
 for(const [i,b] of [...document.querySelectorAll('#seattle-categories button')].entries())sectionSearchLabel(b,seattleSections()[i]);
 const scopeLabel=['vienna','prague','copenhagen'].includes(presentation)?'all categories':genre|| (favoritesOnly?'Favorites':presentation==='cambridge'&&cambridgeSection==='recent'||presentation==='ulm'&&ulmSection==='recent'?'Recently played':'All games');
 for(const field of [$('#search'),$('#ulm-search')]){field.placeholder='Search '+scopeLabel;field.setAttribute('aria-label','Search '+scopeLabel);}
 const restoreSearchFocus=document.activeElement?.matches('.show-all-matches');
 const grid=$('#grid');grid.querySelectorAll('.search-empty').forEach(n=>n.remove());grid.classList.remove('search-empty-grid');document.body.classList.remove('search-no-results');
 const overview=presentation==='vienna'||presentation==='prague',active=overview?(presentation==='vienna'?viennaCurrent():pragueCurrent()):null;
 const activeEmpty=overview&&active&&!active.items.length;
 if(!searchSummary||filtered.length&&!activeEmpty||presentation==='ulm'&&ulmLevel==='categories')return;
 const elsewhere=searchSummary.all,scope=activeEmpty?(active.key==='favorites'?'Favorites':'this category'):genre?'this category':favoritesOnly?'Favorites':presentation==='cambridge'&&cambridgeSection==='recent'||presentation==='ulm'&&ulmSection==='recent'?'Recently played':null;
 const panel=el('div','search-empty');panel.setAttribute('role','status');
 panel.append(el('p','',elsewhere&&scope?'No matches in '+scope+'.':'No matching titles in your library.'));
 if(elsewhere){panel.append(el('p','',elsewhere.toLocaleString()+' '+(elsewhere===1?'match':'matches')+' elsewhere.'));const button=el('button','secondary show-all-matches',overview?'Go to matching category':'Show all matches');button.onclick=showAllSearchMatches;panel.append(button);}
 grid.querySelectorAll('.cambridge-empty,.ulm-empty,.seattle-empty,.expanded .vienna-empty,.expanded .prague-empty').forEach(n=>n.remove());grid.classList.add('search-empty-grid');if(!filtered.length)document.body.classList.add('search-no-results');
 const host=presentation==='seattle'?grid.querySelector('.seattle-shelf'):presentation==='vienna'?grid.querySelector('.expanded .vienna-row'):presentation==='prague'?grid.querySelector('.expanded .prague-list'):grid;
 (host||grid).append(panel);$('#empty').hidden=true;if(restoreSearchFocus)panel.querySelector('button')?.focus({preventScroll:true});
}
