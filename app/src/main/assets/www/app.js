...
function card(g){const b=el('button','game');b.dataset.id=g.id;b.setAttribute('aria-label','Play '+g.title);const cover=el('div','cover'),im=el('img');im.src=g.image;im.alt='';im.loading='lazy';im.decoding='async';im.width=480;im.height=720;cover.append(im);b.append(cover,el('span','game-title',g.title),el('span','game-genre',g.genre));b.addEventListener('click',()=>{if(presentation!=='library'&&presentation!=='seattle'&&presentationId!==g.id){selectPresentation(g.id);return}play(g.id,b)});b.addEventListener('contextmenu',e=>{e.preventDefault();openGame(g.id,b)});const edit=el('span','card-edit','•••');edit.setAttribute('role','button');edit.setAttribute('aria-label','Details for '+g.title);edit.tabIndex=0;edit.addEventListener('click',e=>{e.stopPropagation();openGame(g.id,b)});edit.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();e.stopPropagation();openGame(g.id,b)}});b.append(edit);syncFavoriteCard(b,g);return b}
...
function persist(){if(native&&!restoring)native.saveView(JSON.stringify({favoritesOnly,genre,query,sort,size,palette,presentation,presentationId,seattleBrowse,soundEnabled,artFit,typography,coverGlow,visible,scroll:window.scrollY,focus:presentation!=='library'?presentationId:document.activeElement?.closest('.game')?.dataset.id||lastTrigger?.dataset.id||null}))}
...
window.onNativeResume=()=>{if(native?.drawerApps)native.drawerApps();if(lastTrigger&&document.contains(lastTrigger))lastTrigger.focus({preventScroll:true})};
...
window.controller=action=>{document.querySelector('.skip')?.classList.remove('keyboard-access');stopCarouselMotion();
 ...
 if(action==='search'){if(!modal)controllerFocus($('#search'));return}
...
let saved={};try{saved=native?JSON.parse(native.view()):{}}catch{}restoring=true;initApps();initFavorites();initPresentation();presentation=['kyoto','cupertino','tokyo','seattle'].includes(saved.presentation)?saved.presentation:'library';presentationId=saved.presentationId||saved.focus||null;seattleBrowse=saved.seattleBrowse===true;document.body.dataset.presentation=presentation;$('#presentation').value=presentation;setPalette(saved.palette);soundEnabled=saved.soundEnabled!==false;$('#sound-enabled').checked=soundEnabled;setArtFit(saved.artFit);initAppearance(saved);rebuildGenres();favoritesOnly=saved.favoritesOnly===true;genre=favoritesOnly?'':genres.includes(saved.genre)?saved.genre:'';query=saved.query||'';sort=['az','za','recent'].includes(saved.sort)?saved.sort:'az';size=['compact','comfortable','large'].includes(saved.size)?saved.size:'comfortable';visible=Math.max(72,Math.min(saved.visible||72,games.length));$('#search').value=query;$('#sort').value=sort;setSize(size);update();setTimeout(()=>{window.scrollTo(0,saved.scroll||0);if(saved.focus){const selectedCard=document.querySelector('.game[data-id="'+saved.focus+'"]');if(selectedCard){controllerGameId=saved.focus;document.body.dataset.input='controller';selectedCard.classList.add('controller-selected');selectedCard.focus({preventScroll:true})}}restoring=false;focusStartupGame()},200);
