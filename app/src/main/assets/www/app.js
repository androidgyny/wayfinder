...
function persist(){if(native&&!restoring)native.saveView(JSON.stringify({favoritesOnly,genre,query,sort,size,palette,presentation,presentationId,seattleBrowse,viennaKey,viennaPositions,soundEnabled,artFit,typography,coverGlow,backdrop,selectionStyle,cupertinoReflections,cupertinoSpacing,berlinCoverSize,kyotoTitlePlacement,cupertinoTitlePlacement,visible,scroll:window.scrollY,focus:presentation!=='library'?presentationId:document.activeElement?.closest('.game')?.dataset.id||lastTrigger?.dataset.id||null}))}
