...
function card(g){const b=el('button','game');b.dataset.id=g.id;b.setAttribute('aria-label','Play '+g.title);const cover=el('div','cover'),im=el('img');im.src=g.image;im.alt='';im.loading='lazy';im.decoding='async';im.width=480;im.height=720;cover.append(im);b.append(cover,el('span','game-title',g.title),el('span','game-genre',g.genre));b.addEventListener('click',()=>{if(menuButtons==='selected'&&document.body.dataset.input==='touch'&&['library','seattle'].includes(presentation)&&menuTouchId!==g.id){menuTouchId=g.id;controllerFocus(b,true);return}if(presentation==='vienna'){viennaClick(g,b);return}if(presentation!=='library'&&presentation!=='seattle'&&presentationId!==g.id){selectPresentation(g.id);return}play(g.id,b)});b.addEventListener('contextmenu',e=>{e.preventDefault();openGame(g.id,b)});const edit=el('span','card-edit','•••');edit.setAttribute('role','button');edit.setAttribute('aria-label','Details for '+g.title);edit.tabIndex=0;edit.addEventListener('click',e=>{e.stopPropagation();openGame(g.id,b)});edit.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();e.stopPropagation();openGame(g.id,b)}});b.append(edit);syncFavoriteCard(b,g);return b}
...
function persist(){if(native&&!restoring)native.saveView(JSON.stringify({favoritesOnly,genre,query,sort,size,palette,presentation,presentationId,seattleBrowse,viennaKey,viennaPositions,soundEnabled,artFit,typography,coverGlow,backdrop,backgroundDim,selectionStyle,cupertinoReflections,cupertinoSpacing,berlinCoverSize,kyotoTitlePlacement,cupertinoTitlePlacement,viennaSeparators,coverCorners,coverBrightness,menuButtons,appearancePresets,visible,scroll:window.scrollY,focus:presentation!=='library'?presentationId:document.activeElement?.closest('.game')?.dataset.id||lastTrigger?.dataset.id||null}))}
