...
public class MainActivity extends Activity {
    ...
    private AmbientAudio ambient; private Artwork artwork; private File covers; private String pickingId=""; private boolean ready=false; private long lastAxis=0;
    ...
    private JSONArray pendingAppPreferences; private JSONArray pendingImport; private File pendingDir;
    ...
    @Override public void onCreate(Bundle state){
        ...
        covers=new File(getFilesDir(),"covers");covers.mkdirs();artwork=new Artwork(this);db=new Library();db.getWritableDatabase();
    ...
    @Override protected void onPause(){if(ambient!=null)ambient.leave();stopDirection();if(startup!=null)startup.dismiss();if(web!=null){web.evaluateJavascript("window.saveView&&window.saveView()",null);web.onPause();}leftTriggerKey=rightTriggerKey=leftTriggerAxis=rightTriggerAxis=false;super.onPause();}
    @Override protected void onDestroy(){if(ambient!=null)ambient.destroy();if(startup!=null)startup.dismiss();if(web!=null){web.removeJavascriptInterface("Portal");web.destroy();web=null;}if(sounds!=null){sounds.release();sounds=null;}worker.shutdown();db.close();super.onDestroy();}
    ...
    @Override public boolean dispatchKeyEvent(KeyEvent e){int k=e.getKeyCode();String action=null;
        ...
        if(action!=null&&ready){if(e.getAction()==KeyEvent.ACTION_DOWN&&((!action.equals("favorite")&&!action.equals("apps"))||e.getRepeatCount()==0))key(action);return true;}return super.dispatchKeyEvent(e);
    }
    ...
    private Intent target(JSONObject g) throws Exception {
        ...
        else{intent=getPackageManager().getLaunchIntentForPackage(pkg);if(intent==null)throw new IOException("This app is not installed or has no launch screen");}
    ...
    public class Bridge {
        ...
        @JavascriptInterface public void setThemeColor(String color){if(color!=null&&color.matches("#[0-9a-fA-F]{6}"))runOnUiThread(()->{int value=Color.parseColor(color);getWindow().setStatusBarColor(value);getWindow().setNavigationBarColor(value);View decor=getWindow().getDecorView();int flags=decor.getSystemUiVisibility(),light=View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR|View.SYSTEM_UI_FLAG_LIGHT_NAVIGATION_BAR;boolean bright=(Color.red(value)*.2126+Color.green(value)*.7152+Color.blue(value)*.0722)>160;decor.setSystemUiVisibility(bright?flags|light:flags&~light);});}
    ...
    private void importZip(Uri uri){notice("Checking backup…");File stage=new File(getCacheDir(),"restore-"+UUID.randomUUID());stage.mkdirs();try(ZipInputStream zip=new ZipInputStream(getContentResolver().openInputStream(uri))){JSONArray records=null,apps=null;ZipEntry entry;int total=0;Set<String> names=new HashSet<>();while((entry=zip.getNextEntry())!=null){String name=entry.getName();if(entry.isDirectory())continue;if(!names.add(name))throw new IOException("Duplicate backup entry");if(!name.equals("library.json")&&!name.matches("user/[a-zA-Z0-9-]+\\.jpg"))throw new IOException("Unexpected backup file");ByteArrayOutputStream out=new ByteArrayOutputStream();byte[] buf=new byte[32768];int n,count=0;while((n=zip.read(buf))!=-1){count+=n;total+=n;if(count>16000000||total>512000000)throw new IOException("Backup exceeds size limit");out.write(buf,0,n);}if(name.equals("library.json")){JSONObject backup=obj(out.toString("UTF-8"));if(!backup.optString("format").equals("portal-library")||(backup.optInt("version")!=1&&backup.optInt("version")!=2))throw new IOException("Unsupported backup format");records=backup.getJSONArray("games");apps=backup.optJSONArray("apps");if(backup.optInt("version")==2&&apps==null)throw new IOException("Missing app preferences");}else{try(FileOutputStream f=new FileOutputStream(new File(stage,name.substring(5)))){out.writeTo(f);}}}
        ...
        pendingAppPreferences=apps;pendingImport=records;pendingDir=stage;final int count=records.length();runOnUiThread(()->new AlertDialog.Builder(this).setTitle("Restore library?").setMessage("Replace this library with the "+count+" games in the backup? "+(pendingAppPreferences!=null?"App pins, appearance and app history will also be restored. ":"")+"Your installed games and their save data are not changed.").setNegativeButton("Cancel",(d,w)->clearStage()).setOnCancelListener(d->clearStage()).setPositiveButton("Restore",(d,w)->worker.execute(()->{try{File[] files=pendingDir.listFiles();if(files!=null)for(File f:files)Files.copy(f.toPath(),new File(covers,f.getName()).toPath(),java.nio.file.StandardCopyOption.REPLACE_EXISTING);db.replace(pendingImport);if(pendingAppPreferences!=null)getPreferences(0).edit().putString("apps",pendingAppPreferences.toString()).commit();emit("restored",new JSONObject());}catch(Exception e){notice("Restore failed: "+e.getMessage());}finally{clearStage();}})).show());
    }catch(Exception e){File[] fs=stage.listFiles();if(fs!=null)for(File f:fs)f.delete();stage.delete();notice("Could not restore: "+e.getMessage());}}
    private void clearStage(){if(pendingDir!=null){File[] f=pendingDir.listFiles();if(f!=null)for(File x:f)x.delete();pendingDir.delete();}pendingDir=null;pendingImport=null;pendingAppPreferences=null;}
